...
#else
int ak49xx_irq_of_init(struct device_node *node,
...
static int ak49xx_irq_probe(struct platform_device *pdev)
{
	...
	if (irq < 0) {
		...
		if (unlikely(!domain)) {
			ret = ak49xx_irq_of_init(pdev->dev.of_node, NULL);
			if (ret)
				return ret;
			domain = irq_find_host(pdev->dev.of_node);
			if (unlikely(!domain)) {
				pr_err("%s: failed to create irq domain", __func__);
				return -EINVAL;
			}
		}
...
static int ak49xx_irq_probe(struct platform_device *pdev)
{
	...
	if (!gpio_is_valid(irq)) {
		...
		if (unlikely(!domain)) {
			/*
			 * The original 3.18 tree relies on the IRQCHIP init pass to
			 * create this domain before the platform driver is probed.  That
			 * ordering is not guaranteed by the 4.4-based rebase, so make
			 * the probe self-sufficient.
			 */
			ret = ak49xx_irq_of_init(pdev->dev.of_node, NULL);
			if (ret)
				return ret;
