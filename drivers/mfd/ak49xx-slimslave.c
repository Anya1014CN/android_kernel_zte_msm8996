		   ...
		   unsigned int rx_num, unsigned int *rx_slot)
{
	...
	int i;
	/*
	 * The channel tables belong to the codec driver.  They are reused for
	 * every route change, so an allocation failure must never free them:
	 * doing so leaves the DAPM virtual-port controls with dangling list
	 * nodes and turns the next playback/capture route into a kernel crash.
	 */
	if (!ak49xx || !tx_slot || !rx_slot || !tx_num || !rx_num)
		return -EINVAL;
