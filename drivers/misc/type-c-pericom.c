...
static const unsigned int piusb_extcon_cables[] = {
	EXTCON_USB,
	EXTCON_USB_HOST,
	EXTCON_NONE,
};

static const u32 piusb_extcon_exclusive[] = { 0x3, 0 };
...
{
	/* power-supply current properties are expressed in microamps. */
	const union power_supply_propval ret = {limit * 1000,};

	/* Update USB of max charging current (500 mA corresponds to BC1.2). */
...
static void piusb_update_extcon(struct pi_usb_type_c *pi)
{
	...
	extcon_set_cable_state_(pi->extcon, EXTCON_USB_HOST, host);
}
...
static int piusb_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	pi_usb->extcon = devm_extcon_dev_allocate(&i2c->dev,
						piusb_extcon_cables);
	if (IS_ERR(pi_usb->extcon)) {
		ret = PTR_ERR(pi_usb->extcon);
		dev_err(&i2c->dev, "failed to allocate extcon device: %d\n", ret);
		goto i2c_disable;
	}
	pi_usb->extcon->mutually_exclusive = piusb_extcon_exclusive;
	ret = devm_extcon_dev_register(&i2c->dev, pi_usb->extcon);
	if (ret) {
		dev_err(&i2c->dev, "failed to register extcon device: %d\n", ret);
		goto i2c_disable;
	}
