...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
...
#else
	prop = of_find_property(np, "synaptics,vddio-gpio", NULL);
	if (prop && prop->length) {
		bdata->vddio_gpio = of_get_named_gpio_flags(np,
				"synaptics,vddio-gpio", 0, NULL);
	} else {
		bdata->vddio_gpio = -1;
	}
#endif
	...
	SYNA_INFO("cuixiaochuan: %s, %d, power_gpio: %d, vddio_gpio: %d, irq_gpio: %d, reset_gpio: %d\n",
		__func__, __LINE__, bdata->power_gpio, bdata->vddio_gpio, bdata->irq_gpio, bdata->reset_gpio);
...
EXPORT_SYMBOL(synaptics_rmi4_bus_exit_2nd);
