			     ...
			     int enable, bool dapm)
{
	...
	if (IS_ERR_OR_NULL(ak4962)) {
		dev_err(codec->dev, "%s: codec is not initialized\n", __func__);
		return -ENODEV;
	}

	if (!IS_ERR_OR_NULL(ak4962->cdc_ext_clk)) {
...
static int ak4962_restore_slim_channels(struct ak49xx *core,
					struct ak4962_priv *ak4962_p);
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	mutex_lock(&codec->component.card->mutex);
	if ((!core || !core->rx_chs || !core->tx_chs) &&
		ak4962_restore_slim_channels(core, ak4962_p)) {
		dev_err(codec->dev, "%s: unable to restore SLIM channels\n", __func__);
		goto err;
	}
	if (!core || !core->tx_chs || dai_id >= NUM_CODEC_DAIS ||
		port_id >= AK4962_TX_MAX) {
		dev_err(codec->dev, "%s: invalid TX DAI %u port %u\n", __func__,
			dai_id, port_id);
		goto err;
	}
	...
	case SB3_CAP:
		/*
		 * A TX virtual port may be reassigned as audio routes are brought
		 * up.  Keep each node on at most one DAI list; the vendor path
		 * relied on a global bitmap and could leave a node linked twice.
		 */
		if (enable) {
			if (!list_empty(&core->tx_chs[port_id].list))
	...
	mutex_unlock(&codec->component.card->mutex);
...
/*
 * Some Android 8 era SSR/routing paths drop the AK49xx channel-table
 * pointers after codec probe.  Android's audio HAL may bring up its first
 * route later, so recreate the exact fixed fujisan SLIM map on demand.
 */
static int ak4962_restore_slim_channels(struct ak49xx *core,
					struct ak4962_priv *ak4962_p)
{
	int i;
	static const unsigned int rx_slots[AK4962_RX_MAX] =
		{138, 139, 140, 141, 142, 143};
	static const unsigned int tx_slots[AK4962_TX_MAX] =
	...
	if (!ak4962_p)
		return -EINVAL;
	if (core->rx_chs && core->tx_chs)
		return 0;

	rx_chs = kmalloc(sizeof(ak4962_rx_chs), GFP_KERNEL);
	tx_chs = kmalloc(sizeof(ak4962_tx_chs), GFP_KERNEL);
	if (!rx_chs || !tx_chs) {
		kfree(rx_chs);
		kfree(tx_chs);
		return -ENOMEM;
	...
	memcpy(rx_chs, ak4962_rx_chs, sizeof(ak4962_rx_chs));
	memcpy(tx_chs, ak4962_tx_chs, sizeof(ak4962_tx_chs));
	/* The old table may have been freed while its nodes remained linked. */
	for (i = 0; i < NUM_CODEC_DAIS; i++)
		INIT_LIST_HEAD(&ak4962_p->dai[i].ak49xx_ch_list);
	core->rx_chs = rx_chs;
	core->tx_chs = tx_chs;
	core->num_rx_port = AK4962_RX_MAX;
	core->num_tx_port = AK4962_TX_MAX;

	ret = ak49xx_init_slimslave(core, core->slim->laddr,
				    AK4962_TX_MAX, (unsigned int *)tx_slots,
				    AK4962_RX_MAX, (unsigned int *)rx_slots);
	if (ret) {
		core->rx_chs = NULL;
		core->tx_chs = NULL;
		core->num_rx_port = 0;
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	...
	/*
	 * Route initialisation can move an RX virtual port between AIFs. The
	 * vendor callback assumed the port was only ever added once; adding an
	 * already linked node corrupts the DAI list and crashes validation.
	 */
	if ((!core || !core->rx_chs || !core->tx_chs) &&
		ak4962_restore_slim_channels(core, ak4962_p)) {
		dev_err(codec->dev, "%s: unable to restore SLIM channels\n", __func__);
	...
	if (!core || !core->rx_chs || port_id >= AK4962_RX_MAX) {
		dev_err(codec->dev, "%s: invalid RX port %u\n", __func__, port_id);
		goto err;
	}

	if (!list_empty(&core->rx_chs[port_id].list))
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	pr_debug("%s: event called! codec name %s num_dai %d\n"
		...
		__func__, snd_soc_dapm_to_codec(w->dapm)->component.name, snd_soc_dapm_to_codec(w->dapm)->component.num_dai, w->sname, event);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	pr_debug("%s: event called! codec name %s num_dai %d stream name %s\n",
		__func__, snd_soc_dapm_to_codec(w->dapm)->component.name, snd_soc_dapm_to_codec(w->dapm)->component.num_dai, w->sname);
...
static int ak4962_codec_probe(struct snd_soc_codec *codec)
{
	...
	if (!of_find_property(ak49xx->dev->of_node, "clock-names", NULL)) {
		...
		if (IS_ERR(cdc_ext_clk)) {
			ret = PTR_ERR(cdc_ext_clk);
			dev_err(codec->dev, "%s: clk get %s failed: %d\n",
					__func__, "cdc_ext_clk", ret);
			cdc_ext_clk = NULL;
				 ...
				 enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_PREPARE:
		...
		if (codec->component.dapm.bias_level == SND_SOC_BIAS_ON) {
		}
		break;
	...
	case SND_SOC_BIAS_STANDBY:
		...
		if (codec->component.dapm.bias_level >= SND_SOC_BIAS_PREPARE) {
		}
		break;
