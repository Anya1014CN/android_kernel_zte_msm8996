					...
					int enable, bool dapm)
{
	...
	#endif
	#ifdef CONFIG_AK4962_CODEC
	ret = ak4962_codec_mclk_enable(codec, enable, dapm);
	#endif
				 ...
				 struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		return msm_snd_enable_codec_ext_clk(snd_soc_dapm_to_codec(w->dapm), 1, true);
	case SND_SOC_DAPM_POST_PMD:
		return msm_snd_enable_codec_ext_clk(snd_soc_dapm_to_codec(w->dapm), 0, true);
	}
	return 0;
}
...
static int msm_hifi_ctrl_event(struct snd_soc_dapm_widget *w,
				    struct snd_kcontrol *k, int event)
{
	struct snd_soc_card *card = snd_soc_dapm_to_codec(w->dapm)->component.card;
	struct msm8996_asoc_mach_data *pdata =
				snd_soc_card_get_drvdata(card);
	int ret = 0;
...
/*  */
	...
	/* start mbhc */
	err = snd_soc_card_jack_new(codec->component.card, "Headset Jack",
...
#ifdef CONFIG_AK4961_CODEC
	err = snd_soc_card_jack_new(codec->component.card, "Button Jack",
					AK4961_JACK_BUTTON_MASK, &button_jack, NULL, 0);
	if (err) {
		pr_err("failed to create new jack\n");
		return err;
	}
...
#ifdef CONFIG_AK4962_CODEC
	err = snd_soc_card_jack_new(codec->component.card, "Button Jack",
					AK4962_JACK_BUTTON_MASK, &button_jack, NULL, 0);
	if (err) {
		pr_err("failed to create new jack\n");
		return err;
	}
...
/*
 * The original ZTE initializer also installs legacy controls and MBHC while
 * the 4.4 card is still assembled.  Those pieces are not safe to restore
 * wholesale, but the AK4962 cannot carry media PCM until its SlimBus channel
 * map and AFE slave configuration have been registered with ADSP.
 */
static int msm8996_ak4962_card_init(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_codec *codec = rtd->codec;
	struct snd_soc_dapm_context *dapm = &codec->component.dapm;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
	unsigned int rx_ch[AK4962_RX_MAX] = {138, 139, 140, 141, 142, 143};
	unsigned int tx_ch[AK4962_TX_MAX] = {128, 129, 130, 131, 132, 133,
					       134, 135, 136, 137};
	int ret;
	...
	if (ret < 0) {
		pr_err("%s: failed to set AK4962 SlimBus map: %d\n",
		       __func__, ret);
		return ret;
	}
	...
	if (ret < 0)
		pr_err("%s: failed to set AK4962 AFE config: %d\n",
		       __func__, ret);
...
static struct snd_soc_dai_link msm8996_ak4962_be_dai_links[] = {
	/* Backend DAI Links */
	{
		...
		.dpcm_playback = 1,
		.be_id = MSM_BACKEND_DAI_SLIMBUS_0_RX,
		/* The original 3.18 init routine registers legacy controls, MBHC
		 * and ADSP callbacks while this 4.4 card is still being assembled.
		 * That corrupts the ASoC card registration path on msm8996.  Keep
		 * this active AK4962 link on the 4.4-safe card initializer. */
		.init = &msm8996_ak4962_card_init,
		.be_hw_params_fixup = msm_slim_0_rx_be_hw_params_fixup,
		/* this dainlink has playback support */
		.ignore_pmdown_time = 1,
...
static struct snd_soc_card *populate_snd_card_dailinks(struct device *dev)
{
	...
	card->num_links = msm8996_ak496x_prune_unavailable_links(dailink,
							 len_4);
